...
In the current implementation of SDL with WiFi transport, IP address and port is shared with Mobile Apps. However, the prerequisite for using SDL with WiFi is that user should connect to WiFi manually.
In some cases entring password for the access point created by HMI could be tedious and it would be lot more convinient for users if Mobile Apps could automatically connect to Wi-Fi access point.

If WiFi accesspoint is automatically generated by HMI, the password could contain alphanumric or special characters or it could be too long. It would be inconvinient for user, to enter such passwords in HMI. Also, to improve the security of network, it is recommended to keep chaning passwords. User would need to connect to WiFi network every time password is changed.

## Proposed solution

The available transport type for Secondary Transport can be configured through smartDeviceLink.ini file. 

If `MultipleTransportsEnabled` is set as `true` and `SecondaryTransportForBluetooth` is `WiFi` in `smartDeviceLink.ini` configuration file. HMI should create a WiFi Access Point and these WiFi creadentials should be communicated to proxy.

Android and iOS provides API's to connect to WiFi network in case WiFi SSID and password is known. This API's could be used in Mobile Proxy to connect to Wi-Fi access point created by HMI. The flow for the proposed solution would be:

`HMI creates WiFi Access Point --> HMI provides WiFi Access point credentials to SDL Core --> SDL Core shares WiFi credentials using Primary Transport --> Mobile Proxy receives WiFi access point credentails and check if its already connected --> If not connected, Mobile Proxy automatically connects to WiFi accesspoint.`
...
WiFi credentials will be communicated to proxy using `Transport Event Update` control frame. `Transport Event Update` frame will be changed to include following parameters:
...
wifiPassword | string | (Optional) Specify a WiFi password to connect speified WiFi access point.
...
On receiving `Transport Event Update` with WiFi credentials, Proxy should check if mobile device is already connected to WiFi accesspoint mentioned in `Transport Event Update`. If mobile device is not already connected, it should try connecting to WiFi access point. 

On successful connection or if mobile device is already connected to WiFi accesspoint mentioned in `Transport Event Update`, core and proxy should follow use case described in proposal [Supporting simultaneous multiple transports](https://github.com/smartdevicelink/sdl_evolution/blob/master/proposals/0141-multiple-transports.md "Supporting simultaneous multiple transports").
...
2. Android App will need to include WiFi permsissions in Manifest file, if they support WiFi as secondary transport.
...
Below code can be used to connect to Wi-Fi Access point if SSID and password is known.
