...
To make vehicle data APIs (_GetVehicleData/SubscribeVehicleData/UnsubscribeVehicleData/OnVehicleData_) more flexible to allow addition of new vehicle data items while maintaining the security access via policies.
...
```
* VehicleDataItem - It is the vehicle data item in question. e.g. gps, speed etc. SDL would use this as vehicle data param for requests from App and to validate policies permissions
* SDLDataType - Return data type of the vehicle data item. It can be generic SDL data type e.g. Integer, String, Float etc OR an Enumeration defined in APIs XML. For vehicle data item which has sub-params, this would be Struct.
* OEMDataRef - This is a reference for OEM mapping table which defines signal attributes for this vehicle data items. OEMs may use this table to differentiate between various vehicle and SW configurations. SDL will pass along this reference to HMI, HMI would be responsible to resolve this reference using OEM specific mapping table.
...
We will keep a base JSON schema file for SDL core and proxy locally and another one in the OEM cloud. OEMs can update the schema file with OEM specific items which SDL core would be able to download over the cloud along with policy update and SDL proxy would be able to download via a URL passed to the app from HMI as an _endpoint_ shown below:
```
"endpoints": {
    "0x07": {
...
* Instead of using JSON to define vehicle data items schema, we can use either of following:
  * XML:
    * This would reuse the XML based design with adding "reference" as another attribute or as a sub-element. Instead of sending a JSON file from ARMR to SYNC we would send an XML file that would be like a subset of the mobile API. 
    * But this would need changes at PT server side as Policy server is configured to serve JSON as part of PT update.
  * Precompiled library plug-ins:
    * Another idea is to automatically generate code and compile this code into a dynamic link library. Instead of sending a JSON or XML file this library would be transferred to SYNC as part of policy update or OTA. Core would load this library and execute this library for vehicle data.
    * But this might cause overhead to maintain and support dlls for multiple OSs.
