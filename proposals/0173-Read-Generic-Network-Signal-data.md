...
* OEM's schema file should always be baselined with latest version of standardized schema file from open SDL. OEM can then further add OEM specific custom vehicle data items. This ensures that any version of core which implement this proposal would be able to process the latest version of standardized vehicle data items schema, hence would be able to support all future additions for new standardized vehicle data items.
