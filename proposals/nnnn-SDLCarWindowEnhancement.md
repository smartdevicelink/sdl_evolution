...
If the developer wishes to handle touch events in their app, they can adopt a protocol provided by SDL, such as   [SDLTouchManagerDelegate](https://github.com/smartdevicelink/sdl_evolution/blob/master/proposals/0090-SDLHapticHitTesterProtocol.md). 
...
- (void)sdl_didReceiveTouchEvent:(NSNotification *)notification {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.touchDelegate != nil) {
            CGPoint touchLocation = [targetView convertPoint:locationInWindow fromView:nil];
            if ([self.touchDelegate didReceiveTouchForView:targetView atPoint:touchLocation] == YES) {
                return; // Delegate handled touch
            }
        }
