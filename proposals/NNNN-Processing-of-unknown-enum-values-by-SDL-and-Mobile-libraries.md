  ...
  * Proposal: [F129](F128-Processing-of-unknown-enum-values-by-SDL-and-Mobile-libraries.md)
  * Author: [Igor Gapchuk](https://github.com/IGapchuk), [Dmitriy Boltovskiy](https://github.com/dboltovskyi/), [Kostiantyn Boskin](https://github.com/kostyaboss), [Yurii Lokhmatov](https://github.com/yoooriii)
  * Status: Review
...
To ensure backward compatibility with earlier versions of SDL core, mobile libraries should be updated to process, filter and format enums inside of RPC requests when a difference in RAPI sets have been detected.
  ...
  When SDL receives a request from Mobile Application it starts validation of all incoming parameters.
  ...
  3. Parameter is part of the structure (see example 3)
      SDL has to proceed the same way as in case #1 or #2 and remove this parameter from the structure.
      ...
      During this process, SDL has to proceed recursively from the very bottom level up to the top.
  ...
  **Examples:**
  ...
  1. Request contains a parameter which value is an enum item
      ...
      SDL omits empty parameter from the request.
  ...
  2. Request contains a parameter which value is an array of enum items
